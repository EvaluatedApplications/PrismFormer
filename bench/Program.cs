...
if (args.Contains("--swarmconverge")) { SwarmDemoBench.RunConverge(args); return; }   // logged convergence curve: merged swarm vs single-process, loss/acc fall to a floor AND max-param-diff = 0.0 (bit-identical) at every checkpoint
if (args.Contains("--avgconverge")) { SwarmDemoBench.RunAvgConverge(args); return; }   // logged convergence curve for the WEIGHT-AVERAGING path: divergence shrinks to a small NONZERO floor (consensus, lossy) while cross-skill accuracy bleeds in
...
if (args.Contains("--capacity")) { CapacityBench.Run(maxN: maxNArg ?? 256, passes: stepsArg ?? 400, tuned: true); return; }   // atomic capacity: equal-sized transformer vs PrismFormer memorise the most unique arbitrary key→value facts (recall-vs-N, LINEAR sweep 32..maxN step 32)
