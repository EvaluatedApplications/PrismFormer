...
public static class ColumnarBench
{
    ...
    public static void Run(int seeds = 4, int steps = 15000)
    {
        const int ctx = 20;
        var evalN = 300;

        Console.WriteLine("PrismFormer worked-arithmetic probe — each operation in isolation, show your working\n");
        ...
        Parallel.ForEach(jobs, job =>
        {
            ...
            void TrainOne(Action<int[], int> step)
            {
                ...
                for (var i = 0; i < steps; i++)
                {
                    ...
                    for (var t = start; t < full.Length; t++) step(full[..t], full[t]);
                }
            }
            if (job.kind == 1)
            {
                var m = new MiniTransformer(vocab: V, dModel: 104, dff: 208, layers: 4, maxT: ctx, seed: 300 + sd);
                TrainOne((t, a) => m.TrainStep(t, a, 1e-3)); xfs[job.oi][job.seed] = m;
            }
            else
            {
                ...
                TrainOne((t, a) => m.TrainStep(t, a, 1e-3));
                if (job.kind == 0) algs[job.oi][job.seed] = m; else algsU[job.oi][job.seed] = m;
            }
        });

        Console.WriteLine($"params: PrismFormer {algs[0][0].ParamCount:N0}   transformer {xfs[0][0].ParamCount:N0}\n");
