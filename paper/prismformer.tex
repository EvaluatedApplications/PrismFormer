...
split into shards and summed is \textbf{bit-for-bit identical} to the serial result. I verify that merge
property to the bit, and it is what a \textbf{full-replication training swarm} would be built on, every
node holding the whole model on a plain CPU and staying mathematically coherent across mismatched
hardware, unlike sharded systems that need matched accelerators. I am careful about what I have and have
not shown: the exact merge is a \emph{proven property} and the swarm is the \emph{architecture} it
enables and a direction I argue for, \textbf{not} yet a demonstrated distributed-training run, since every
experiment here is single-process. In that single-process setting I do show skill spreading without a
coordinator: independently-initialised nodes share what they have learned through continuous weight
coupling, and the shared codec makes that sharing near-lossless ($99\%$ versus $75\%$ cross-init skill
transfer).
...
system ($99\%$ versus $75\%$). Every experiment here is single-process, so I present this as a proven
merge-and-share property plus the architecture it enables, and flag the live multi-node run as future
...
the merge property made concrete across separate instances, not just a checksum. It is still a single
process (a live multi-node run is future work, Section~\ref{sec:lim}; the over-the-wire bit-exact version
runs in \code{PrismNet swarmtest}). Run \code{dotnet run --project bench -c Release -- --swarmdemo}.
