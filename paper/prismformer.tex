...
archived as a frozen snapshot alongside this document (repository tag \texttt{paper-v2}); every
...
accelerators. Skill spreads across that swarm without a coordinator: independently-initialised nodes
share what they have learned through continuous weight coupling, and the shared codec makes that sharing
near-lossless ($99\%$ versus $75\%$ cross-init skill transfer).
...
I did not start from the literature. I started from a short list of axioms that are frankly philosophy,
not engineering, and I would rather say that plainly than dress it up: that consciousness is the one
thing you cannot doubt without using it, that reasoning has to be free of contradiction, and that the
act of observation \emph{generates} distinctions, each new distinction opening a dimension, so that the
space of concepts only ever grows and never collapses. I do not defend any of that here, and the paper
does not rest on it. I mention it because it is the true starting point, and because it put a very
specific picture in my head.

The picture was a \emph{Platonic space}, and I want to be careful with the credit, because the idea is
ancient, it is Plato's, and Michael Levin, whose talks are where I met it, is himself explicit that he
did not invent it. What I took from Levin's framing, his conversation with Lex Fridman and his own
writing \parencite{levin2025lex,levin_platonic_blog}, is the operational version: a structured,
non-physical space of patterns, the properties of mathematical objects among them, that \emph{in-forms}
the physical world rather than emerging from it, with a mind acting as an \emph{interface} onto that
space rather than a thing that manufactures it. I do not need any of that to be literally true. I needed
one thing from it: that a concept can have a definite, deterministic address, the way the number five,
or a particular word, has exactly one place it belongs, so that reasoning becomes moving between
addresses under rules rather than lookup in a table. That is the whole of what I borrowed.

The intuition that fixed this for me is an old one: do we all see red as the same colour? The Platonic
framing answers yes, not because anyone's private experience is provably identical to anyone else's, but
because \emph{red} is a defined point in the space and each of us is pointing at the same one. I do not
claim to have settled that for human minds. I claim only that I built a system where it is true by
construction: in the codec, \emph{red}, or \emph{five}, or any token, resolves to one deterministic
address, so every copy of the model agrees on where it lives. That agreement is not decoration. It is
exactly what later lets a swarm of separate machines merge into one coherent model, because they are all
writing to the same coordinates.
...
\item \textbf{A full-replication CPU swarm that shares skill (Sections~\ref{sec:merge}
and~\ref{sec:skillshare}).} Because every node holds the whole model and gradients merge exactly, a swarm
of mismatched CPUs trains one coherent model, the opposite design to sharded systems such as Petals
\parencite{borzunov2023petals}, which split one model across matched accelerators and lose it if a peer
drops. And skill spreads without a coordinator: independently-initialised nodes share what they have
learned through continuous weight coupling, near-losslessly when they share the codec's coordinate system
($99\%$ versus $75\%$).
\end{enumerate}
...
tasks and seeds. Two honest caveats. The two fair tasks not shown, antonym and capital, are tested only
...
Here \emph{face-decode} is the answer read directly off the frozen identity bands with no trained probe,
the legibility measure quantified in Section~\ref{sec:inside}. PrismFormer computes all four; the
transformer does none of them. Given the identical worked problem it
...
\code{dotnet run --project bench -c Release -- --average}.
...
\textbf{The swarm, actually distributed.} I show that sharded gradients merge bit-for-bit, that the
mechanism runs, and that skill spreads across independently-initialised nodes through continuous coupling
(Section~\ref{sec:skillshare}), but every experiment here is single-process. A genuine multi-node run,
with a convergence curve, a fault-injection test, and a wall-clock scaling measurement, is future work;
until it exists, the swarm should be read as a demonstrated \emph{merge-and-share property} plus an
\emph{architecture}, not a demonstrated distributed-training result. The value of bit-exactness over ordinary asynchronous
...
snapshot (repository tag \texttt{paper-v2}), so it does not depend on the live repository, which
...
result is produced by the reproducibility kit (frozen snapshot, tag \texttt{paper-v2}) and can be re-run
